...
import com.fox.culture.dao.MemberDAO;
...
public class MemberMain extends JFrame {
	...
	JButton jbmselect;
	JButton[] buttons;
	String[] btnTitle = {"입력","수정","삭제","검색","전체보기","아이디중복"};
	
	JTextField tfSearch, tfmsearch;
	Checkbox cbmname, cbmid;
	...
	Object[] columnNames = {"아이디","비밀번호","이름","생년월일","주소","전화번호","가입일자"};
	//데이타베이스에 접속해서 DML작업을 위한 DAO계열 클래스 변수 선언
	MemberDAO dao;

	JTextField tfId,tfName,tfBirth,tfAddr,tfHp;
	...
	private void createComponent() {		
		...
		//체크박스 생성
		jbmselect  = new JButton("검색");
		cbmname = new Checkbox ("회원이름", true);
		cbmid = new Checkbox ("ID");
	...
	private void setTableData(int flag) {
		...
		for(int i=0; i < list.size();i++){
			...
			rowData[i][0]= dto.getMid();
			rowData[i][1]= dto.getMpw();
			rowData[i][2]= dto.getMname();
			rowData[i][3]= dto.getMbirth();
			rowData[i][4]= dto.getMaddr();	
			rowData[i][5]= dto.getMhp();	
			rowData[i][6]= dto.getMdate();
		}
...
class MyHandler extends MouseAdapter implements ActionListener{
	...
	public static void main(String[] args) {
		...
		MemberMain csMain = new MemberMain();
	}
